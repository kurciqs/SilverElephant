...
def setup(width, height):
    ...
    # img = Image.open("silverelephant/res/icon.png")
    # glfw.set_window_icon(CANVAS.W, 1, img)
...
def beginFrame():
    time.sleep(0.000001)
    glfw.poll_events()
    CANVAS.mouse_L_pressed = bool(glfw.get_mouse_button(CANVAS.W, glfw.MOUSE_BUTTON_1))
    CANVAS.mouse_R_pressed = bool(glfw.get_mouse_button(CANVAS.W, glfw.MOUSE_BUTTON_2))
...
def endShape(connect=True):
  ...
  if STROKE.stroke:
    if connect:
        ...
        glLineWidth(STROKE.strokeWeight)
        glColor(STROKE.strokeCol)
        for i in range(len(MESHES.curVertices) - 1):
            glBegin(GL_LINES)
            glVertex2f(MESHES.curVertices[i][0], MESHES.curVertices[i][1])
            glVertex2f(MESHES.curVertices[i+1][0], MESHES.curVertices[i+1][1])
            glEnd()
